...
const VALUES = [
  {
    ...
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round" className="size-5 text-fg-muted" aria-hidden="true">
...
export default function AboutPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-16 space-y-24">
      ...
      {/* Hero */}
      <section>
        <p className="text-xs font-medium uppercase tracking-[0.1em] text-accent mb-4">The Moment</p>
        ...
        <p className="text-fg-muted text-base leading-relaxed max-w-2xl">
          더모먼트는 광주소프트웨어마이스터고등학교 재학생들로 구성된 개발 팀입니다.
          주로 학교에 필요한 서비스를 개발하고 있어요. 본교 입학지원시스템부터 교내 학생의 정보를
          관리하는 서비스, 프로젝트 관리 서비스를 만들어요.
        </p>
      ...
      <section>
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          {VALUES.map((v) => (
            <div key={v.keyword} className="p-6 rounded-xl border border-border bg-bg-subtle flex flex-col gap-3">
              <p className="text-xs font-medium tracking-widest text-fg-muted uppercase">{v.keyword}</p>
              {v.icon}
              <h3 className="text-sm font-semibold text-fg">{v.title}</h3>
              <p className="text-xs text-fg-muted leading-relaxed whitespace-pre-line">{v.description}</p>
              <div className="flex flex-wrap gap-1.5 mt-auto pt-2">
                {v.tags.map((tag) => (
                  <span key={tag} className="text-[10px] text-fg-muted font-medium">{tag}</span>
                ))}
              </div>
            </div>
          ))}
            ...
            >
              ...
              <div className="p-4 flex items-start justify-between gap-2">
                <div>
                  ...
                  <p className="text-xs text-fg-muted mt-0.5 leading-relaxed">{project.description}</p>
                </div>
          ...
          {groups.map(({ role, members }) => (
            <div key={role}>
              <p className="text-xs text-fg-muted mb-4 flex items-center gap-2">
                <span className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-medium ${ROLE_COLOR[role as Role]}`}>
